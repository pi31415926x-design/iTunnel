  ...
  <div class="h-full flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-900">
    ...
    >
      <div v-if="notification.show" :class="[
        ...
        <svg v-if="notification.type === 'success'" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="p-6 pb-0 flex-shrink-0">
      <div class="mb-6 flex items-start justify-between">
        <div>
          ...
          <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">Manage your tunnel configuration and peer settings.</p>
        </div>
        <button
        ...
        >
          <svg v-if="!isSaving" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
      ...
      <div class="max-w-3xl mx-auto">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-6">
            ...
            <form @submit.prevent="saveAllSettings" class="space-y-5">
              ...
              <div class="space-y-1.5">
                <label for="address" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  Address
                ...
                <p v-else class="text-xs text-slate-500 dark:text-slate-400">The IP address for this interface (e.g., 10.0.0.1/24)</p>
              </div>

              <!-- ListenPort -->
              <div class="space-y-1.5">
                <label for="listenPort" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  Listen Port
                  <span class="text-red-500">*</span>
                </label>
                <input
              ...
              <!-- PrivateKey -->
              <div class="space-y-1.5">
                <label for="privateKey" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  Private Key
                  <span class="text-red-500">*</span>
                </label>
                <div class="relative">
                ...
                <p v-else class="text-xs text-slate-500 dark:text-slate-400">Base64 encoded private key (click "Generate Key" to create one)</p>
              </div>
              ...
              <div class="pt-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <label v-for="toggle in [{id:'isTcp', label:'TCP Mode', desc:'Use TCP instead of UDP'}, {id:'isServer', label:'Server Mode', desc:'Act as server'}, {id:'isGlobal', label:'Global Mode', desc:'Route all traffic'}]" :key="toggle.id" 
                    ...
                    <input type="checkbox" v-model="(form as any)[toggle.id]" class="mt-0.5 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 bg-white dark:bg-slate-900">
                    <div class="flex-1">
                      <span class="text-sm font-medium text-slate-700 dark:text-slate-300 block">{{ toggle.label }}</span>
        ...
        <div v-else-if="activeTab === 'peers'" class="animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-6">
            ...
            <form @submit.prevent="saveAllSettings" class="space-y-5">
              <div class="space-y-1.5">
                <label for="publicKey" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  Public Key
                  <span class="text-red-500">*</span>
                </label>
                <input 
                  id="publicKey" 
                  v-model="peerForm.publicKey"
                  @blur="validatePeerField('publicKey')"
                  type="text" 
                  placeholder="Peer's base64 encoded public key"
                  :class="[
                ...
                <p v-else class="text-xs text-slate-500 dark:text-slate-400">The peer's public key for authentication</p>
              </div>
              ...
              <div class="space-y-1.5">
                ...
                <div class="relative">
                  <input 
                    id="presharedKey" 
                    v-model="peerForm.presharedKey"
                    @blur="validatePeerField('presharedKey')"
                    :type="showPresharedKey ? 'text' : 'password'"
                    placeholder="Optional preshared key for additional security"
                    :class="[
                  ...
                  <button
                    type="button"
                    @click="showPresharedKey = !showPresharedKey"
                    class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
                  >
                ...
                <p v-else class="text-xs text-slate-500 dark:text-slate-400">Additional layer of symmetric encryption (recommended)</p>
              </div>

              <div class="space-y-1.5">
                <label for="allowedIPs" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  Allowed IPs
                  <span class="text-red-500">*</span>
                </label>
                <input 
                  id="allowedIPs" 
                  v-model="peerForm.allowedIPs"
                  @blur="validatePeerField('allowedIPs')"
                  type="text" 
                ...
                <p v-else class="text-xs text-slate-500 dark:text-slate-400">Comma-separated CIDR ranges (e.g., 0.0.0.0/0 for all traffic)</p>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label for="endpoint" class="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                    Endpoint
                    <span class="text-red-500">*</span>
                  </label>
                  <input 
                    ...
                    @blur="validatePeerField('endpoint')"
                    type="text" 
                ...
                <div class="flex items-end">
                  <label class="flex items-start space-x-3 cursor-pointer p-4 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 w-full hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors">
                    <input type="checkbox" v-model="peerForm.isChangeRoute" class="mt-0.5 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500">
                    <div>
        ...
        <div v-else-if="activeTab === 'others'" class="animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-12 text-center shadow-sm">
            <div class="mx-auto w-16 h-16 bg-indigo-50 dark:bg-indigo-900/30 rounded-full flex items-center justify-center mb-6">
...
const saveAllSettings = async () => {
  ...
  if (isInterfaceValid && isPeerValid) {
    ...
    try {
      const response = await fetch('/setwg', {
        method: 'POST',
        headers: {
